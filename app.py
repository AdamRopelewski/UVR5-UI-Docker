        ...
        return (
            ...
            + "".join(
                ...
                for i, line in enumerate(
                    re.findall(
                        r"^(?!-+)(.+)$", result.stdout.strip(), re.MULTILINE
                    )
                )
            )
            + "</table>"
    ...
    if total_files == 0:
        ...
        for audio_files in found_files:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Roformer batch separation failed: {e}"
                ) from e
...
with gr.Blocks(
    ...
    title="🎵 UVR5 UI Dockerized 🎵",
) as app:
    gr.Markdown("<h1> 🎵 UVR5 UI Dockerized 🎵 </h1>")
    gr.Markdown(
        i18n(
            "If you like UVR5 UI you can star my repo on [GitHub](https://github.com/AdamRopelewski/UVR5-UI-Docker)"
        )
    )
    with gr.Tabs():
        with gr.TabItem("BS/Mel Roformer"):
                ...
                with gr.Group():
                    ...
                    with gr.Row():
                        roformer_overlap = gr.Slider(
                            label=i18n("Overlap"),
                            info=i18n(
                                "Amount of overlap between prediction windows"
                            ),
                            minimum=2,
                            maximum=10,
                            step=1,
                    ...
                    with gr.Row():
                        roformer_single_stem = gr.Textbox(
                            ...
                            placeholder=i18n(
                                "Write the stem you want, check the stems of each model on Leaderboard. e.g. Instrumental"
                            ),
                            interactive=True,
                        )
            with gr.Row():
            ...
            with gr.Accordion(i18n("Separation by link"), open=False):
                ...
                with gr.Row():
                    roformer_download_button = gr.Button(
                        i18n("Download!"), variant="primary"
                    )

            roformer_download_button.click(
                download_audio, [roformer_link], [roformer_audio]
            )
            ...
            with gr.Accordion(i18n("Batch separation"), open=False):
                ...
                with gr.Row():
                    roformer_bath_button = gr.Button(
                        i18n("Separate!"), variant="primary"
                    )
                with gr.Row():
        ...
        with gr.TabItem("MDX23C"):
            ...
            mdx23c_download_button.click(
                download_audio, [mdx23c_link], [mdx23c_audio]
            )
        ...
        with gr.TabItem("MDX-NET"):
                ...
                with gr.Group():
                    with gr.Row():
                        mdxnet_hop_length = gr.Slider(
                            ...
                            info=i18n(
                                "Usually called stride in neural networks; only change if you know what you're doing"
                            ),
                            minimum=32,
                            maximum=2048,
                            step=32,
            ...
            mdxnet_download_button.click(
                download_audio, [mdxnet_link], [mdxnet_audio]
            )
        ...
        with gr.TabItem("VR ARCH"):
            ...
            vrarch_download_button.click(
                download_audio, [vrarch_link], [vrarch_audio]
            )
        ...
        with gr.TabItem("Demucs"):
                ...
                with gr.Group():
                    with gr.Row():
                        ...
                        demucs_segment_size = gr.Slider(
                            label=i18n("Segment size"),
                            info=i18n(
                                "Size of segments into which the audio is split. Higher = slower but better quality"
                            ),
                            minimum=1,
                            maximum=100,
                            step=1,
                    ...
                    with gr.Row():
                        demucs_overlap = gr.Slider(
                            label=i18n("Overlap"),
                            info=i18n(
                                "Overlap between prediction windows. Higher = slower but better quality"
                            ),
                            minimum=0.001,
                            maximum=0.999,
                            step=0.001,
            ...
            demucs_download_button.click(
                download_audio, [demucs_link], [demucs_audio]
            )
            ...
            demucs_model.change(
                update_stems, inputs=[demucs_model], outputs=stem6
            )
            ...
            with gr.Group():
                with gr.Row(equal_height=True):
                    ...
                    list_button = gr.Button(
                        i18n("Show list!"), variant="primary"
                    )
            output_list = gr.HTML(label=i18n("Leaderboard"))

            list_button.click(
                leaderboard, inputs=list_filter, outputs=output_list
            )
